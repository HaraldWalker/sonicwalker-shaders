/*{
    ...
    "DESCRIPTION": "Datamosh riot — chunky color-band signal tearing with row shifts, RGB split, block corruption and scanlines",
    "INPUTS": [
        {
            "NAME": "sliceSize",
            "LABEL": "Slice Size",
...
void main() {
    ...
    float scan = 0.5 + 0.5 * sin(shiftedCoord.y * RENDERSIZE.y * 1.4 + t * 4.0);
    scan = mix(1.0, scan, scanAmt);
    col *= scan;

    float jitter = (hash(vec2(texCoord.x * RENDERSIZE.x, seedC)) - 0.5) * 0.08 * glitchStrength;
    col += jitter;
